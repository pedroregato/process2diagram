...
def render_mermaid_block(mermaid_text: str, *, show_code: bool = True, key_suffix: str = "") -> None:
    ...
    Estratégia híbrida:
      1. Tenta buscar SVG via mermaid.ink (server-side)
      2. Se obtiver SVG, injeta com pan/zoom via components.html
      3. Se falhar, exibe imagem estática via st.image como fallback
    """
    import hashlib
    import base64
    ...
    # ── Tentar obter SVG do mermaid.ink (server-side) ────────────────────
    svg_content = None
    fetch_error = None
    try:
...
if(!w||!h){var vb=s.viewBox?s.viewBox.baseVal:null;if(vb&&vb.width>0){w=vb.width;h=vb.height}}
if(!w||!h){w=s.scrollWidth||800;h=s.scrollHeight||600}
var cW=c.clientWidth,cH=c.clientHeight;
scale=Math.min(cW/w,cH/h,2)*0.9;
...
fitToScreen();
    ...
    # ── Caminho B: fallback — imagem estática via mermaid.ink URL ─────────
    else:
