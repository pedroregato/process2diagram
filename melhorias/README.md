...
`arquivados/` também guarda propostas anteriores a esta taxonomia (pré-2026-08-01) cujo status individual não foi reclassificado nesta rodada — a maioria já é implementação fechada, mas algumas foram avaliadas e descartadas dentro do próprio documento (ex.: seções de uma proposta maior). Não assuma "arquivado = implementado" para os arquivos anteriores a essa data sem conferir o conteúdo.
