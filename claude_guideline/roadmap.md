...
- **Tabela `feedback`** (`setup/supabase_migration_feedback.sql`) — linhas cruas por evento (não pré-agregada, diferente do `feedback_aggregate` da proposta original que mantinha médias em JSONB via read-modify-write) — mesmo padrão de `llm_telemetry`/`provocations`: agregação calculada na leitura via `get_feedback_summary()`, mirror de `get_roster_attendance_summary()` já existente. `rating` sempre 1-5 mesmo pro widget de thumbs (down→1, up→5) — uma única escala pros dois tipos de widget.
- **`ui/components/artifact_feedback.py::render_artifact_feedback()`** — ⭐ 1-5 + "aceitável?" + comentário opcional, mesmo padrão comprovado de `promote_asset.py` (toggle `st.button()` + `st.container()`, nunca `st.expander()`/`st.popover()` — evita a classe de bug "Bad 'setIn' index" já documentada). Plugado em `pages/Artefatos.py`: aba BPMN (por processo, ao lado do botão de promoção) e aba Reuniões (por ata, dentro do expander da reunião).
- **Thumbs (👍/👎) em `pages/Assistente.py`** — após cada resposta do Assistente no chat, usando o índice do loop como `artifact_id` (histórico não tem UUID por mensagem); "já avaliada" rastreado em `st.session_state` (não sobrevive a reload — aceitável pro v1).
- **`diagnostico_projeto()` ganha 8º check** (`include_feedback=True`, também exposto no schema OpenAI da tool) — `bpmn_process`/`meeting_minutes` com `acceptance_rate < 0.7` → alerta/crítico + ação; `assistant_response` com `avg_rating < 3.0` → alerta. Guard de volume mínimo (3 avaliações) contra falso positivo, mesmo princípio de `detect_error_anomalies()` (PC183).
- **Não implementado, deliberadamente:** `AgentMeta`, `pages/MetaDashboard.py`, `services/meta_scheduler.py`, qualquer mecanismo de auto-apply — Camadas 2 e 3 da proposta original.
- 14 testes novos (`tests/test_artifact_feedback.py`) — `save_feedback`/`get_feedback_summary` fail-open + agregação, `diagnostico_projeto()` com feedback sintético em cada faixa (crítico/alerta/ok/volume insuficiente/desligado por flag). Boot-smoke real via `AppTest` em `pages/Artefatos.py` (toggle de feedback abre o formulário — estrelas + checkbox — sem exceção, nos dois pontos de integração); `pages/Assistente.py` verificado por leitura de código + syntax check (AppTest completo tem prerequisitos extensos de sessão, mesmo trade-off já aceito no PC189/PC190).
- Migration `setup/supabase_migration_feedback.sql` criada mas **não executada automaticamente** — mesmo padrão do PC190, pendente de confirmação do usuário.
- **Achado colateral corrigido:** `setup/supabase_migration_provocations.sql` (PC190) estava com o conteúdo local sobrescrito por uma nota de confirmação ("Migration executado com sucesso.") — histórico do git estava intacto (commit `1470cfa`), só a cópia da working tree tinha sido clobbered; restaurado via `git checkout --`.
