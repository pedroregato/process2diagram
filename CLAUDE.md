...
│   ├── Orientacoes_ComoIniciar.py   # Guia de início rápido (Ajuda)
│   ├── Orientacoes_CasosDeUso.py    # Casos de Uso — Valor de Negócio: 15 cenários (Cenário → Pergunta → Entrega → Valor) em 5 categorias, fonte única alimenta Streamlit + botão de export HTML autocontido (PC181) (Ajuda)
│   ├── Orientacoes_Assistente.py    # Guia de ferramentas do Assistente (151 tools + exemplos) (Ajuda)
│   ├── Orientacoes_Glossario.py     # Glossário interativo (components.v1.html — busca + filtros + índice alfabético) (Ajuda)
│   ├── Capacitacao.py               # Curso corporativo — módulos hands-on por cenário de negócio (Ajuda)
│   ├── Orientacoes_Arquiteturas.py  # Arquiteturas do sistema (Guias)
│   ├── Orientacoes_CKF.py           # Guia CKF (Guias)
│   ├── Orientacoes_BpmnStudio.py    # Guia BPMN Studio — passo a passo + exemplo de descrição complexa (multi-pool) (Guias)
│   ├── Orientacoes_Graficos.py      # Guia dos 13 gráficos do Assistente — o que são, melhor uso, prompts + resumo executivo (PC143) (Guias)
│   ├── Orientacoes_CacheSemantico.py # Guia do cache LLM (hash exato SHA-256, PII-safe) — conceito, arquitetura, por que não é fuzzy/embedding (PC185) (Guias)
│   ├── Orientacoes_Feedback.py   # Guia de Avaliação/Feedback (PC191) — onde avaliar, fluxo até diagnostico_projeto(), valor de negócio, o que NÃO faz (sem auto-apply) (Guias)
│   ├── Orientacoes_Manifesto.py  # Manifesto de Engenharia — princípios arquiteturais (Guias)
...
│   │   ├── promote_asset.py      # render_promote_button() — toggle+container pattern (não expander), promoção a Ativo de Negócio
│   │   └── artifact_feedback.py  # render_artifact_feedback() (PC191) — ⭐ 1-5 + aceitável + comentário, mesmo padrão toggle+container de promote_asset.py
...
Telemetria passiva em todo `_call_llm()` real (não cache); daemon thread assíncrono; tabela `llm_telemetry` (90d cleanup). `TelemetryRecord`: agent_name, provider, model, latency_ms, tokens_in/out, from_cache, long_context, is_error, error_message, benchmark_run, skill_version, is_validation_event, schema_valid, project_id, meeting_id, approved_count, rejected_count, rejected_reasons. `run_benchmark_call()` para benchmarks on-demand. `BENCHMARK_TASKS` (5 agentes) + `TRANSCRIPTS` (curta/média). `pages/LLMBenchmark.py`: 🧪 Benchmark On-Demand + 📊 Telemetria Real (box plot latência, throughput, histórico, heatmap, versões, 🚨 alertas, ✅ qualidade). Migration: `setup/supabase_migration_llm_telemetry.sql` ✅ + `setup/supabase_migration_llm_telemetry_pc183.sql` ✅ + `setup/supabase_migration_llm_telemetry_pc207.sql` ✅.

**PC183 — erro por provider + qualidade de schema:** `_call_llm()` agora envolve a chamada real ao provider em `try/except`: falha grava `TelemetryRecord(is_error=True, error_message=...)` e relança a exceção original (retry/escalação de `_call_with_retry()` intocados) — antes disso, `is_error` era hard-coded `False` e nenhuma falha real (ex.: DeepSeek "conteúdo vazio") deixava rastro. `_call_with_retry()` persiste o resultado de `output_schema.model_validate()` (PC84) via `_telemetry.record_validation()` — antes só existia como `warnings.warn()` efêmero. `LLMTelemetry`: `query_error_rate_by_provider(hours)`, `detect_error_anomalies(hours, min_calls, error_rate_threshold)`, `query_recent_errors(hours, limit)`, `query_schema_validation_rate(days, agent_name)`, `record_validation(agent_name, skill_version, valid)`.

**PC207 — observabilidade de Provocações por reunião:** `project_id`/`meeting_id` genéricos no schema (não exclusivos de Provocações, mas só esta rodada os popula) — sem eles, `llm_telemetry` só respondia perguntas agregadas ("taxa de erro do provider X"), não "por que ESTA reunião não gerou provocações". `core/pipeline.py::run_provocations()` chama `_telemetry.record_provocations_outcome(project_id, meeting_id, skill_version, approved_count, rejected_count, rejected_reasons)` depois de cada execução de `AgentProvocations` — persiste o que antes só existia em `hub.provocations` (efêmero) e num `logging.info()` nos logs do processo. `LLMTelemetry.query_provocations_diagnostics(project_id, meeting_id=None, limit=5)` + tool do Assistente `get_provocations_diagnostics(meeting_number)` (não-admin, categoria "consulta").
...
- `session_state.init_session_state()` — idempotent, call immediately after `st.set_page_config()`. Defaults: provider=DeepSeek, embed_provider=OpenAI, run_quality/bpmn/minutes/requirements=True, run_sbvr/bmm/synthesizer/dmn/argumentation/ckf_updater/query_summarizer=True, n_bpmn_runs=3, use_langgraph=True, enable_long_context=True. `run_provocations=True` (PC205 — era `False`/opt-in desde o PC190; ligado por padrão desde que testes em projeto real mostraram que, desligado, a feature nunca gera dado pra validar) — custa 1 chamada LLM extra por reunião.
