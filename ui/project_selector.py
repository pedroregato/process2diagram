...
# ─────────────────────────────────────────────────────────────────────────────
# Seleção / criação de contexto e metadados da reunião.
...
from core.project_store import list_contexts, create_context, list_bpmn_processes

# Context types available for new contexts
_CONTEXT_TYPES = [
    "projeto",
    "produto",
    "domínio",
...
def render_context_selector() -> None:
    ...
    if not supabase_configured():
        st.info("💡 Supabase não configurado — execução sem persistência de contexto.", icon="ℹ️")
        return
    ...
    with st.expander("📁 Contexto / Reunião — obrigatório para salvar resultados",
        ...
        new_ctx_type = "projeto"
        if sel == _NEW:
            col_n, col_s, col_d = st.columns([2, 1, 2])
            with col_n:
                new_name = st.text_input("Nome do contexto *", key="new_proj_name",
                                         placeholder="Nome completo do contexto")
            with col_s:
            ...
            with col_d:
                new_desc = st.text_input("Descrição (opcional)", key="new_proj_desc",
                                          placeholder="Objetivo ou escopo do contexto")
            new_ctx_type = st.selectbox(
                "Tipo de contexto",
                _CONTEXT_TYPES,
                key="new_proj_ctx_type",
                help="Categoriza o contexto para personalizar métricas e análises",
            )

        if st.button("✅ Confirmar contexto", key="confirm_project_btn",
                     type="primary", use_container_width=True):
            _handle_confirm(projects, sel, new_name, new_sigla, new_desc, title, meeting_dt, new_ctx_type)


def _handle_confirm(projects, sel, new_name, new_sigla, new_desc, title, meeting_dt, new_ctx_type="projeto"):
    if not title.strip():
    ...
    if sel == _NEW:
        if not new_name.strip():
            st.error("Informe o nome do novo contexto.")
            return
        ...
        if not project:
            st.error("Contexto não encontrado.")
            return
...
render_project_selector = render_context_selector
