...
**Fora de escopo ainda (decisão consciente, não lacuna silenciosa):** NAV-04 (fonte única do contexto ativo — reescrita de `require_active_project()`/`ui/project_selector.py`) e NAV-05 (sessão sobrevive a F5 — tabela nova + mecanismo de cookie/token) fecham a Onda 1 mas pedem decisão de design própria (ex.: cookie `HttpOnly` vs `st.query_params` para o token de sessão); NAV-06 a NAV-15 (Ondas 2-3, reorganização de menu/desempenho/consistência) não reavaliadas item a item. Retomar começando por NAV-04/05.
