# pages/DmnBackfill.py
# ─────────────────────────────────────────────────────────────────────────────
# DMN Backfill — extrai e persiste tabelas de decisão DMN (dmn_json)
# para reuniões processadas antes do AgentDMN existir no pipeline.
# ─────────────────────────────────────────────────────────────────────────────
...
# ── Header ────────────────────────────────────────────────────────────────────
st.markdown("# ⚖️ DMN Backfill")
st.caption(
    "Extrai e persiste tabelas de decisão DMN para reuniões processadas "
    "antes do AgentDMN existir no pipeline."
)
...
sel_proj     = st.selectbox("Contexto", list(proj_map.keys()), index=_default_idx, key="dmn_bf_proj")
if not sel_proj:
    st.stop()
...
try:
    all_meetings = (
        ...
        .select(
            ...
            "transcript_clean, transcript_raw, dmn_json"
        )
...
show_all = st.checkbox(
    "Mostrar todas as reuniões (inclusive as que já têm DMN)",
    value=False,
    key="dmn_bf_show_all",
)

def _missing(m: dict) -> bool:
    return not m.get("dmn_json")
...
if not pending:
    if show_all:
        ...
        st.success("✅ Todas as reuniões do projeto já possuem DMN.")
    st.stop()

col_a, col_b = st.columns(2)
col_a.metric("Reuniões com transcrição", len(all_with_transcript))
col_b.metric("Sem DMN" if not show_all else "Exibindo", len(pending))

def _status(m: dict) -> str:
    return "✅ DMN" if m.get("dmn_json") else "❌ sem DMN"
...
if st.button(
    f"▶️ Extrair DMN para {len(pending_sel)} reunião(ões)",
    type="primary",
    key="dmn_bf_run",
):
    ...
    for i, meeting in enumerate(pending_sel):
        ...
        try:
            ...
            n_decisions = len(getattr(hub.dmn, "decisions", []))
