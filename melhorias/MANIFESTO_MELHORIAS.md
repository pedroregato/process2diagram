...
**Fora de escopo desta rodada (decisão consciente, não lacuna silenciosa):** NAV-03 a NAV-15 exigem mudanças maiores — tabela nova + mecanismo de cookie/token para sessão persistente (NAV-05), paginação/refatoração de `ValidationHub.py` e outras 5 páginas pesadas (NAV-03/09), reorganização do menu em `app.py` afetando `st.switch_page`/`st.page_link` em todo o app (NAV-06/08), reescrita de `require_active_project()`/`ui/project_selector.py` (NAV-04) — cada uma pede uma decisão de design própria (ex.: cookie `HttpOnly` vs `st.query_params` para o token de sessão) e testes `AppTest` mais amplos do que o Wave 1 de segurança. Não reavaliado formalmente item a item; retomar quando houver prioridade, começando pela Onda 1 (NAV-03/04/05).
