...
## Propostas triadas fora das rodadas 1/2 (por PC individual, não por auditoria em lote)

### `navegabilidade.md` — 🟡 Implementado (parcial — Onda 1 completa) — `melhorias/parciais/navegabilidade.md`
**Ideia:** auditoria de navegabilidade em produção (41 páginas, contexto SDEA, domínio fgv) — 15 tarefas `NAV-01..15` em 3 ondas (Onda 1 Estabilizar / Onda 2 Organizar / Onda 3 Refinar), cobrindo isolamento de tenant, mensagens de erro de login, paginação de páginas pesadas, fonte única do contexto ativo, persistência de sessão, reorganização de menu, ícones duplicados, tradução PT-BR, deep links, cabeçalho padrão e responsividade.
**Entregue (PC211, 2026-09-23):** **NAV-01** — `list_projects` (alias cego de `list_contexts` sem `tenant_id`) removido de `core/project_store.py`; os 14 pontos de chamada reais corrigidos para `list_contexts(tenant_id=st.session_state.get("_tenant_id"))` (`pages/Diagramas.py`, `CostEstimator.py`, `Pipeline.py`, `Settings.py`, `Capacitacao.py` + as 9 páginas de Manutenção admin-only: `BatchRunner`, `BpmnBackfill`, `DmnBackfill`, `IbisBackfill`, `MinutesBackfill`, `PiiBackfill`, `ProvocationsBackfill`, `RequirementsBackfill`, `TranscriptBackfill`) — 2 chamadas a mais do que o levantamento original da proposta (`Settings.py`/`Capacitacao.py` também vazavam contextos entre tenants). **NAV-02** — `modules/tenant_auth.py::login_tenant_debug()` não devolve mais prefixo de hash em nenhum motivo de falha; `ui/auth_gate.py` mostra sempre mensagem genérica para falha de login multi-tenant, motivo detalhado só via `logging.warning()`. `tests/test_tenant_isolation.py` (6 testes) + `tests/test_auth_messages.py` (3 testes) — 1024 testes no total, sem regressão.
**Entregue (PC212, 2026-09-23):** **NAV-03** — `pages/ValidationHub.py`: filtro de status padrão trocado para "Pendentes"; leituras cacheadas (`st.cache_data(ttl=60)`) e `list_requirements` (join pesado) trocado por `list_requirements_light`; paginação por aba (25/50/100 itens, seletor + Anterior/Próximo); os 3 botões por item (Validar/Em Revisão/Rejeitar) substituídos por `st.data_editor` com coluna Status + botão "Salvar alterações" (grava só as linhas alteradas via `edited_rows`); KPIs do topo trocados de `len()` sobre listas completas para `core.project_store.count_validation_status()` (`count="exact"` agregado, sem transferir conteúdo). `tests/test_validation_hub_pagination.py` (3 testes) + fix de um bug de isolamento pré-existente achado de passagem em `tests/test_project_selector_confirmed_reset.py` (vazava `st.session_state` global entre arquivos de teste) — 1027 testes no total, sem regressão.
**Entregue (PC213, 2026-09-24):** **NAV-04** — `ui/project_selector.py` ganha `get_active_context()`, `_activate_context(ctx)` e `render_active_context_picker(key)` (renomeado de `render_context_selector` — nome já ocupado pelo formulário de confirmação de contexto/reunião do Pipeline, propósito diferente); índice do selectbox sempre calculado a partir de `active_project_id`, troca gravada só via `on_change` (nunca no valor padrão do primeiro render). `require_active_project()` renderiza esse seletor inline em vez de redirecionar para a Central de Operações. `pages/Diagramas.py` (removido `_diag_synced_pid`) e `pages/CostEstimator.py` (antes ignorava `active_project_id`) passam a usar o seletor compartilhado. `tests/test_active_context_picker.py` (3 testes) — 1030 testes no total, sem regressão.
**Entregue (PC214, 2026-09-24):** **NAV-05** — fecha a Onda 1. Tabela nova `user_sessions` (migração executada e verificada em produção) + `core/project_store.py` (`create_user_session`/`validate_user_session`/`update_user_session_context`/`revoke_user_session`, expiração deslizante 12h, fail-open); `modules/tenant_auth.py::get_tenant_user()` rechecagem em tempo real de tenant/usuário ativo na restauração; `modules/session_cookie.py` (novo) via `streamlit_javascript` — **decisão de design:** nem cookie `HttpOnly` (via `extra-streamlit-components`, cogitado no plano original) nem qualquer outra opção é `HttpOnly` de verdade num Streamlit puro (sem hook de `Set-Cookie`); optei por reaproveitar `streamlit_javascript`, já pinado sem uso real no código, em vez de somar uma dependência nova com o mesmo perfil de segurança. `ui/auth_gate.py::_try_restore_session()`/`_maybe_restore_session()` tratam a entrega assíncrona do cookie (2 tentativas — 1ª mostra placeholder neutro, nunca o login, evitando "flash"). `modules/auth.py::logout()` revoga a sessão + limpa o cookie. `ui/project_selector.py::activate_context()` (renomeado de `_activate_context`) passa a também atualizar `last_context_id`; `pages/Home.py` refatorado pra usar esse helper em vez de duplicar a gravação inline. Escopo: só login multi-tenant — login local (`USUARIOS` hardcoded, fallback de emergência) fica efêmero. `tests/test_user_sessions.py` (11 testes) + `tests/test_session_restore.py` (5 testes) — 1046 testes no total, sem regressão. **Não verificado:** timing real do cookie num navegador (só mock determinístico em `AppTest`) — recomendado teste manual pós-deploy. **Fora de escopo:** limpeza automática de sessões expiradas (ficam no banco até expirar, sem job de cleanup).
**Avaliação e priorização da Onda 2 (2026-09-24):** revisão item a item de NAV-06 a NAV-11 contra o código atual (pós NAV-01-05) — 2 achados mudaram a priorização original. **NAV-07 mais sério do que "MÉDIA":** `pages/ReportBackfill.py` não tinha nenhum gate de `is_admin()`, única página tipo-backfill do app fora de Manutenção — qualquer usuário comum disparava `AgentSynthesizer` (custo real de LLM) sem checagem de perfil. **NAV-08 mais caro do que estimado:** `st.Page()` não tem (e não tinha em nenhuma versão) parâmetro de visibilidade por página — `st.navigation(position=...)` existe mas é global, esconde a sidebar inteira, não uma página específica; a proposta original ("confirmar a versão que introduziu") partia de uma premissa que não se sustenta. Ordem executada: NAV-07a (gate+mover, quase-segurança) → NAV-10 (ícones, zero risco) → NAV-11 parcial (2 fixes concretos) → NAV-07b (aba de leitura). NAV-08 fica fora da Onda 2 — ambas as metades bloqueadas (mecanismo de página oculta não existe no Streamlit pinado; a outra metade depende do NAV-12/Onda 3).
**Entregue (PC215, 2026-09-24):** **NAV-07a** — `pages/ReportBackfill.py` movido de "Análise" pra "Manutenção" em `app.py` (dentro do `if _admin:`, mesmo mecanismo de proteção das outras 10 páginas de backfill — nenhuma se autogatilha internamente, então nenhuma checagem interna foi adicionada); título alinhado à convenção das irmãs ("Backfill — Relatório Executivo"); descrição em `get_system_capabilities()` ganha sufixo "(admin)". `tests/test_report_backfill_admin_gate.py` (4 testes) — 1060 testes no total (contagem final da rodada), sem regressão.
**Entregue (PC216, 2026-09-24):** **NAV-10** — 9 ícones trocados em `app.py` pra eliminar as 9 duplicatas confirmadas (Cenários de Custo, Modelagem Formal, Ferramentas do Assistente, Arquiteturas, Guia CKF, Cache LLM, Minutes Backfill, IBIS Backfill, Backfill — Relatório Executivo). `tests/test_app_page_icons_unique.py` (2 testes).
**Entregue (PC217, 2026-09-24):** **NAV-11 (parcial)** — só os 2 itens concretos do plano: `ui/input_area.py` (4 strings em inglês → PT-BR) e `pages/KnowledgeGraph.py` (bloco de ajuda inteiro sem acentuação corrigido + legenda de `entity_type` traduzida pra exibição via `_TYPE_LABEL_PT`, aplicado com `format_func` — valor armazenado continua em inglês). `tests/test_ptbr_accents_nav11.py` (5 testes). Varredura ampla (18 outros arquivos) fica no backlog.
**Entregue (PC218, 2026-09-24):** **NAV-07b** — nova seção "📄 Relatórios das Reuniões" (somente leitura) em `pages/Artefatos.py`, usando `list_reports_by_project()` (já existia, sem consumidor) + `get_report_html()`; caption explícita direcionando geração/regeneração pra um admin. `tests/test_artefatos_reports_tab.py` (3 testes). Fecha o NAV-07 (junto com PC215).
**Fora de escopo ainda (decisão consciente, não lacuna silenciosa):** NAV-06 (reorganizar o menu — maior risco/blast radius da Onda 2, deixado por último de propósito) e NAV-09 (paginação das 5 páginas pesadas — maior esforço, mas padrão já provado 2x — PC178/PC212) não avaliados nesta rodada de "quick wins". NAV-08 fora da onda (ver achado acima). NAV-12 a NAV-15 (Onda 3) não avaliadas.
