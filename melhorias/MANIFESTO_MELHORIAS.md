...
### `navegabilidade.md` — 🟡 Implementado (parcial) — `melhorias/parciais/navegabilidade.md`
...
**Fora de escopo ainda (decisão consciente, não lacuna silenciosa):** NAV-05 (sessão sobrevive a F5 — tabela nova + mecanismo de cookie/token) fecha a Onda 1 mas pede decisão de design própria (ex.: cookie `HttpOnly` vs `st.query_params` para o token de sessão); NAV-06 a NAV-15 (Ondas 2-3, reorganização de menu/desempenho/consistência) não reavaliadas item a item. Retomar começando por NAV-05.
