...
- **Torneio multi-run (`n_bpmn_runs > 1`) e LangGraph adaptativo:** decidir se o Studio expõe esses controles avançados (que existem no pipeline normal) ou roda sempre em modo simples — recomendação: **modo simples na v1** (sem torneio, sem LangGraph), adicionar como opção avançada só se o uso demonstrar necessidade.
