...
fix_extract_llm.py
──────────────────
Run this script from the ROOT of your process2diagram repository:

    python fix_extract_llm.py

It will overwrite modules/extract_llm.py with the correct content
and print a confirmation with the line count and function list.
"""

import pathlib

TARGET = pathlib.Path("modules/extract_llm.py")

CONTENT = r'''# modules/extract_llm.py
...
if not TARGET.parent.exists():
    print(f"ERROR: Directory '{TARGET.parent}' not found.")
...
assert "def extract_process_llm" in CONTENT
assert "def extract_process_bpmn" in CONTENT
print("\n✅  Both entry points confirmed present.")
print("\nNext step: git add modules/extract_llm.py && git commit -m 'fix: restore extract_llm' && git push")
